...
export const auth = betterAuth({
  ...
  // Persistent 30-day session configuration so candidates stay signed in across
  // browser restarts until explicitly logging out.
  session: {
  ...
  plugins: [
    ...
    magicLink({
      sendMagicLink: async ({ email, token, url }) => {
        console.log(`[MagicLink] Generated magic link for ${email}: ${url}`);
        const apiKey = env("EMAIL_API_KEY");
        const fromAddr = env("EMAIL_FROM") ?? "Jaya Talent <notifications@jayatalent.com>";
        if (apiKey) {
          try {
            await fetch("https://api.resend.com/emails", {
              ...
              body: JSON.stringify({
                from: fromAddr,
                to: [email],
                subject: "✨ Sign in to Jaya Talent — Magic Link",
                html: `
                  <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; padding: 32px; max-width: 480px; margin: 0 auto; background-color: #F8F7F5; border-radius: 16px; border: 1px solid #E5E2DC;">
                    <h2 style="color: #111; font-size: 22px; font-weight: 700; margin-top: 0; margin-bottom: 12px;">Sign in to Jaya Talent</h2>
                    <p style="color: #555; font-size: 15px; line-height: 1.5; margin-bottom: 24px;">Click the link below to instantly sign in to your candidate account. No password required!</p>
                    <a href="${url}" style="display: inline-block; background-color: #111; color: #FFF; text-decoration: none; padding: 14px 28px; border-radius: 12px; font-weight: 600; font-size: 15px;">Sign In to Candidate Dashboard →</a>
                    <p style="color: #888; font-size: 12px; margin-top: 28px; margin-bottom: 0;">This magic link expires in 10 minutes. If you did not request this, you can safely ignore this email.</p>
                  </div>
                `,
              }),
            });
          } catch (err) {
            console.error("[MagicLink] Resend dispatch failed:", err);
          }
        }
      },
    }),
